...
function hasImport(content: string, pkg: string): boolean {
  return (
    new RegExp(`from\\s+['"]${pkg.replace("/", "\\/")}['"]`).test(content) ||
    new RegExp(`require\\s*\\(\\s*['"]${pkg.replace("/", "\\/")}['"]`).test(
      content,
    )
  );
}
...
async function fixWithClaude(
  ...
  if (content.length > 80_000) {
    console.warn(
      `  ${fileName} が大きすぎるためスキップ (${content.length} chars)`,
    );
    return null;
  }
...
修正後のファイル内容のみを返してください（説明なし、マークダウンのコードフェンスなし）。`,
      ...
      if (e?.status === 429 && attempt < 3) {
        ...
        console.warn(
          `  Rate limit (attempt ${attempt}/3), ${wait / 1000}s 待機...`,
        );
...
async function fixRechartsImports(
  ...
  for (const file of tsFiles) {
    ...
    if (
      ...
      const hasStaticRecharts = lines.some((l) =>
        /^import\s+\{[^}]+\}\s+from\s+['"]recharts['"]/.test(l),
      );
      if (!hasStaticRecharts) {
        console.log(
          `  ⏭  ${path.relative(repoDir, file)}: 既にdynamic import済み`,
        );
        continue;
      }
    }
    ...
    console.log(
      `  🔧 recharts dynamic import: ${path.relative(repoDir, file)}`,
    );
...
async function addVercelAnalytics(repoDir: string): Promise<boolean> {
  ...
  if (!fs.existsSync(layoutPath)) {
    console.log(
      `  ⚠️  app/layout.tsx が見つからないため Analytics コンポーネント追加をスキップ`,
    );
    return true;
  }
...
async function removeOpenAI(repoDir: string): Promise<boolean> {
  ...
  for (const file of tsFiles) {
    ...
    console.log(
      `  🔧 openai → @anthropic-ai/sdk: ${path.relative(repoDir, file)}`,
    );
    const fixed = await fixWithClaude(
      path.relative(repoDir, file),
      content,
