...
async function fixSecurityIssues(
  ...
  const fixable = findings.filter(f => f.severity === "critical" || f.severity === "high")
...
async function processRepo(product: any, repo: string) {
  ...
  try {
    ...
    await supabase.schema("metago").from("scores_history").insert({
      product_id: product.id, category: "security", score,
    })

    console.log(`  ✓ ${allFindings.length} findings, score: ${score}`)

    // critical/high 問題 → Claude が修正して L2 PR 作成
    const fixableFindings = allFindings.filter(f => f.severity === "critical" || f.severity === "high")
    if (fixableFindings.length > 0) {
      const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY })
      const { patchCount, summary } = await fixSecurityIssues(repoDir, fixableFindings, product.display_name, anthropic)
...
> ⚠️ L2: 動作確認後に承認してください。セキュリティ修正は機能に影響する可能性があります。`,
          ...
          await supabase.schema("metago").from("approval_queue").insert({
            ...
            description: fixableFindings.map(f => `[${f.severity}] ${f.title}`).join("\n"),
            category:    "security",
            state:       "pending",
