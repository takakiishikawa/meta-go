...
function categorizePackage(name: string): string {
  ...
  if (
    ["date-fns", "dayjs", "lodash", "lodash-es", "uuid", "nanoid", "axios", "qs"].includes(name) ||
...
async function fetchPackageJson(repo: string): Promise<any | null> {
  const content = await githubFetch(`/repos/${GITHUB_OWNER}/${repo}/contents/package.json`)
...
async function collectTechStack(product: any, pkg: any) {
  const deps = Object.entries(pkg.dependencies ?? {}).map(([name, ver]) => ({
    name, ver: String(ver), isDev: false,
  }))
  const devDeps = Object.entries(pkg.devDependencies ?? {}).map(([name, ver]) => ({
    name, ver: String(ver), isDev: true,
  }))
  ...
  const { error } = await supabase
    ...
    .upsert(items, { onConflict: "product_id,package_name", ignoreDuplicates: false })

  if (error) console.error(`tech_stack_items upsert error for ${product.name}:`, error)
  else console.log(`✓ ${product.name}: ${items.length} packages collected to tech_stack`)
}
...
async function collectDependencyUpdates(product: any, pkg: any) {
  ...
  for (const [packageName, currentVersion] of importantPackages) {
    ...
    try {
      const npmData = await fetch(`https://registry.npmjs.org/${packageName}/latest`).then((r) => r.json())
