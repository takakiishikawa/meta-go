...
import { GITHUB_OWNER, GITHUB_TOKEN, REPO_TO_SLUG, listOpenPRs } from "../../lib/github/git-operations"
...
const GO_REPOS = Object.values(REPO_TO_SLUG).reduce(
  (acc, slug) => {
    const repo = Object.entries(REPO_TO_SLUG).find(([r, s]) => s === slug)?.[0] ?? ""
...
async function getProductMap(): Promise<Map<string, string>> {
  const { data: products } = await supabase.schema("metago").from("products").select("id, name")
...
async function syncPRs() {
  ...
  for (const [slug, repo] of Object.entries(GO_REPOS)) {
    ...
    for (const pr of prs) {
      ...
      if (labels.includes("metago-needs-review") && !pendingPrUrls.has(pr.html_url)) {
        ...
        console.log(`  📋 Added to approval_queue: ${pr.title} (${repo}#${pr.number})`)
...
function inferCategory(title: string): string {
  ...
  if (title.includes("デザイン") || title.includes("design")) return "design_system"
  if (title.includes("品質") || title.includes("lint")) return "quality"
  if (title.includes("セキュリティ") || title.includes("security")) return "security"
  if (title.includes("パフォーマンス") || title.includes("performance")) return "performance"
  return "quality"
}
