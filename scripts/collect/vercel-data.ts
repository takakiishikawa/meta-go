...
async function main() {
  ...
  for (const product of products) {
    ...
    const deployments = await vercelFetch(`/v6/deployments?projectId=${projectName}&limit=5`)
    if (!deployments?.deployments?.length) continue

    const latest = deployments.deployments[0]
    console.log(`✓ ${product.display_name}: latest deploy ${latest.state} at ${latest.createdAt}`)
