...
async function collectCareGo(supabase: SupabaseClient) {
  ...
  try {
    ...
    if (checkins && checkins.length > 0) {
      metrics.condition_avg =
        checkins.reduce((a, c) => a + c.score, 0) / checkins.length;
    } else {
...
async function main() {
  console.log(
    `🚀 Collecting Supabase metrics... (psf-snapshot: ${IS_PSF_SNAPSHOT})`,
  );
  ...
  for (const product of products) {
    ...
    // PSFスナップショット（週次のみ）
    if (IS_PSF_SNAPSHOT) {
      const { psf_score, result_score, behavior_score } = calcPsfScore(
        {},
        definitions,
      );
      ...
      console.log(
        `  PSF: ${psf_score.toFixed(1)} (result: ${result_score.toFixed(1)}, behavior: ${behavior_score.toFixed(1)})`,
      );
    }
  }
