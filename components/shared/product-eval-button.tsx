...
export function ProductEvalButton({ items, score, productName }: {
  items: EvalItem[]
  score: number | null
  productName: string
}) {
  ...
  return (
    <>
      ...
      <SimpleDialog open={open} onClose={() => setOpen(false)} title={`${productName} — 定性評価`}>
        <div className="flex flex-col gap-4">
          ...
          {Object.entries(byCategory).length > 0 ? (
              ...
              {Object.entries(byCategory).map(([cat, catItems]) => (
                <div key={cat}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>{catItems.length}件</span>
                  </div>
                  <div className="flex flex-col gap-2 pl-2 border-l-2 border-border">
                    ...
                    {catItems.length > 4 && (
                      <span className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>他 {catItems.length - 4} 件</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="rounded-lg p-4 text-center" style={{ backgroundColor: 'var(--color-surface-subtle)' }}>
              <span className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>現在検出されている問題はありません</span>
            </div>
          )}
        </div>
      </SimpleDialog>
