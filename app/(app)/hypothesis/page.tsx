...
export default async function HypothesisPage() {
  ...
  const [hypothesesRes, backlogRes, productsRes] = await Promise.all([
    supabase.schema("metago").from("hypotheses").select(`*, products(display_name, primary_color)`).order("created_at", { ascending: false }),
    supabase.schema("metago").from("backlog").select(`*, products(display_name, primary_color)`).order("priority").order("created_at", { ascending: false }),
                    ...
                    {problems.map((h) => (
                      <tr key={h.id} className="border-b border-border last:border-0 hover:bg-surface-subtle">
                        ...
                        <td className="px-4 py-3">
                          ...
                          {h.description && (
                            <div className="text-xs mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{h.description}</div>
                          )}
                        </td>
                        <td className="px-4 py-3 text-sm">{h.confidence ? `${h.confidence}%` : "—"}</td>
                        <td className="px-4 py-3"><Badge variant="outline">{h.state}</Badge></td>
                  ...
                  <tbody>
                    {backlog.map((item) => (
                      <tr key={item.id} className="border-b border-border last:border-0 hover:bg-surface-subtle">
                        ...
                        <td className="px-4 py-3"><Badge variant="outline">{item.state}</Badge></td>
                        <td className="px-4 py-3 text-sm" style={{ color: "var(--color-text-secondary)" }}>
