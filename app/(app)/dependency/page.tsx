  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1">
        {items.map((item) => (
          <PackageChip key={item.package_name} name={item.package_name} color={color} />
        ))}
      </div>
    </div>
  )
...
export default async function DependencyPage() {
  ...
  techStack.forEach((item) => {
    pkgProductCount.set(item.package_name, (pkgProductCount.get(item.package_name) ?? 0) + 1)
  })
  ...
  for (const item of techStack) {
    ...
    if (!sharedByCategory.has(item.category)) sharedByCategory.set(item.category, [])
  ...
  for (const item of techStack) {
    if (sharedPkgNames.has(item.package_name)) continue
    if (!stackByProduct.has(item.product_id)) stackByProduct.set(item.product_id, new Map())
            ...
            {sharedByCategory.size > 0 && (
              <div className="rounded-lg border border-border bg-surface overflow-hidden">
                ...
                <div className="px-4 py-4 flex flex-col gap-3">
                  {CATEGORY_ORDER.filter((cat) => sharedByCategory.has(cat)).map((cat) => (
                    <CategorySection
                      key={cat}
                      category={cat}
                      items={sharedByCategory.get(cat)!.map((name) => ({ package_name: name }))}
                    />
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              {allProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    {/* カラーバー */}
                    <div className="h-1" style={{ backgroundColor: product.primary_color ?? "#6B7280" }} />

                    {/* カードヘッダー */}
                    <div className="px-4 py-3 border-b border-border flex items-center gap-2">
                      <span
                        className="size-2 rounded-full shrink-0"
                        style={{ backgroundColor: product.primary_color ?? "#6B7280" }}
                      />
                      <span className="text-sm font-semibold" style={{ color: "var(--color-text-primary)" }}>
                        ...
                        <div className="flex flex-col gap-3">
                          {CATEGORY_ORDER.filter((cat) => catMap!.has(cat)).map((cat) => (
                            <CategorySection
                              key={cat}
                              category={cat}
                              items={catMap!.get(cat)!.map((name) => ({ package_name: name }))}
                            />
                          ))}
                        </div>
                      ) : (
              ...
              <tbody>
                {allItems.map((item) => (
                  <tr key={item.id} className="border-b border-border last:border-0 hover:bg-surface-subtle">
                    <td className="px-4 py-3 text-sm">{(item.products as any)?.display_name ?? "—"}</td>
